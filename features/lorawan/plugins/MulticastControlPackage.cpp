        ...
        switch (_inbound_buf[i++]) {
            ...
            case MCAST_GROUP_CLASS_C_SESSION:
                ...
                if (BIT_SET_TEST(mcast_register->active_mask, group_id)) {
                    ...
                    i += 4;
                    _sessions_ctx.mcast_session[group_id].session_timeout = pow(2.0f, static_cast<int>(_inbound_buf[i++] & 0x0F));
                    ...
                    retcode = cbs->check_params_validity(_sessions_ctx.mcast_session[group_id].dl_freq,
