...
public:
   ...
   /** Sets a given frequency to be used by the stack for RX2 window
     ...
     */
    lorawan_status_t set_rx2_frequency_and_dr(const uint32_t frequency, const uint8_t dr);
     ...
     */
    void restore_rx2_frequency_and_dr(void);
   ...
   /** Adds an entry to the device Multicast register
     ...
     */
    lorawan_status_t register_multicast_address(const mcast_addr_entry_t *entry);

    /** Get access to network provided GPS time
     *
     ...
     *
     * @return Current network assisted GPS time in seconds
     ...
     * current CPU monotonic tick. Please note that the network assisted GPS time
     * is in seconds whereas the monotonic tick is in milliseconds.
     * Having taken snapshot of the current tick would enable us to have a reference
     * to calculate the time difference given the stored GPS time and the forthcoming CPU tick.
     * Please refer to `get_current_gps_time()` API
     ...
     */
    lorawan_mcast_register_t *get_multicast_addr_register(void);
     ...
     */
    lorawan_status_t verify_multicast_freq_and_dr(uint32_t frequency, uint8_t dr);
