                             ...
                             lorawan_time_t *tx_toa)
{
    ...
    tr_debug("Set_Channel %d, Freq = %lu", config->channel, channels[config->channel].frequency);
    _radio->lock();
...
void LoRaPHYUS915::set_fsb_mask()
{
	switch (current_fsb)
	{
		case 1:
			copy_channel_mask(channel_mask, fsb1_channel_mask, US915_CHANNEL_MASK_SIZE);
			copy_channel_mask(current_channel_mask, fsb1_channel_mask, US915_CHANNEL_MASK_SIZE);
			break;
		case 2:
			copy_channel_mask(channel_mask, fsb2_channel_mask, US915_CHANNEL_MASK_SIZE);
			copy_channel_mask(current_channel_mask, fsb2_channel_mask, US915_CHANNEL_MASK_SIZE);
			break;
		case 3:
			copy_channel_mask(channel_mask, fsb3_channel_mask, US915_CHANNEL_MASK_SIZE);
			copy_channel_mask(current_channel_mask, fsb3_channel_mask, US915_CHANNEL_MASK_SIZE);
			break;
		case 4:
			copy_channel_mask(channel_mask, fsb4_channel_mask, US915_CHANNEL_MASK_SIZE);
			copy_channel_mask(current_channel_mask, fsb4_channel_mask, US915_CHANNEL_MASK_SIZE);
			break;
		case 5:
			copy_channel_mask(channel_mask, fsb5_channel_mask, US915_CHANNEL_MASK_SIZE);
			copy_channel_mask(current_channel_mask, fsb5_channel_mask, US915_CHANNEL_MASK_SIZE);
			break;
		case 6:
			copy_channel_mask(channel_mask, fsb6_channel_mask, US915_CHANNEL_MASK_SIZE);
			copy_channel_mask(current_channel_mask, fsb6_channel_mask, US915_CHANNEL_MASK_SIZE);
			break;
		case 7:
			copy_channel_mask(channel_mask, fsb7_channel_mask, US915_CHANNEL_MASK_SIZE);
			copy_channel_mask(current_channel_mask, fsb7_channel_mask, US915_CHANNEL_MASK_SIZE);
			break;
		case 8:
			copy_channel_mask(channel_mask, fsb8_channel_mask, US915_CHANNEL_MASK_SIZE);
			copy_channel_mask(current_channel_mask, fsb8_channel_mask, US915_CHANNEL_MASK_SIZE);
			break;
...
void LoRaPHYUS915::apply_cf_list(const uint8_t *payload, uint8_t size)
{
    ...
    uint16_t temp_channel_mask[US915_CHANNEL_MASK_SIZE] = {0, 0, 0, 0, 0};

    //verify_adr_params_t verify_params;

    // Initialize local copy of channels mask
    copy_channel_mask(temp_channel_mask, channel_mask, US915_CHANNEL_MASK_SIZE);

    for (int i=0; i< US915_CHANNEL_MASK_SIZE; i++)
    {
    ...
    for (int i=0; i< US915_CHANNEL_MASK_SIZE-1; i++)
    {
...
/*
    ...
    // Make active
    copy_channel_mask(channel_mask, temp_channel_mask, US915_CHANNEL_MASK_SIZE);
    copy_channel_mask(current_channel_mask, temp_channel_mask, US915_CHANNEL_MASK_SIZE);
}
                                                ...
                                                lorawan_time_t *aggregate_timeOff)
{
    ...
    /*if (params->current_datarate < 0)
	{
		params->current_datarate = DR_4;
	}*/
	...
	else // Joining mode
	{
		...
		if (params->current_datarate == DR_0)
		{
...
uint8_t LoRaPHYUS915::SelectChannelFromBand(uint8_t fs_band, const uint8_t *enabled_channels, uint8_t nb_enabled_channels)
{
	uint8_t lowerbound = fs_band *8;
    uint8_t higherbound = lowerbound + 7;
    uint8_t channel=0;
    uint8_t channelsinband[US915_MAX_NB_CHANNELS] = {0};
    ...
    if (nb_channelsinband == 0)
    {
    	...
    	{
    		enable_channel(current_channel_mask, i, US915_MAX_NB_CHANNELS);
    	}
